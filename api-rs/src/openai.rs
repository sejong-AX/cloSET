...
pub async fn wardrobe_detect(image_data_url: &str) -> Option<Value> {
    let system = concat!(
        "너는 cloSET의 옷장 정리 어시스턴트다. 사용자가 올린 사진(옷장 전체·행거·개켜 쌓아둔 옷·개별 옷 사진 등)에서 ",
        "보이는 의류를 한 벌씩 분리해 목록으로 만든다. 사람·얼굴·배경·가구·행거 봉·옷걸이는 무시하고 착용 아이템만 담는다. ",
        "액세서리(선글라스·안경·모자·주얼리·시계·벨트·스카프·머플러·장갑·양말 등)는 절대 목록에 넣지 않는다. ",
        "외모·신원 언급 금지. 최대 24개.\n",
        "■ 한 장에 여러 벌이 있을 때 — 반드시 각각을 별개 항목으로 만든다. ",
        "행거에 겹쳐 걸린 옷은 어깨선·소매·색 경계·옷걸이 개수를 세어 구분하고, 앞 옷에 일부 가려졌어도 ",
        "다른 옷이라고 확신되면 포함한다. 개켜서 쌓인 옷더미는 층마다 색·소재 경계로 한 벌씩 센다. ",
        "여러 벌을 하나로 묶거나 '옷 여러 벌' 같은 뭉뚱그린 항목을 만들지 않는다.\n",
        "■ 방향에 의존하지 말 것 — 사진이 90도 회전되어 있을 수 있다. 위아래·좌우가 아니라 ",
        "형태 단서(칼라·소매·목선·허리 밴드·다리통)로 종류를 판단한다.\n",
        "■ 접힌 바지 주의 — 옷걸이 바에 반으로 접혀 걸린 바지는 가로로 넓고 두 겹으로 보여 상의로 오인하기 쉽다. ",
        "허리 밴드·벨트 고리·지퍼/버튼 플라이·뒷주머니·접힘선·나란한 두 개의 다리통·밑단 커프스가 보이면 ",
        "그것은 상의가 아니라 반드시 '하의'다. 반대로 칼라·소매·목선·앞단추 플래킷이 보이면 상의·아우터다.\n",
        "각 항목 필드 — name: 색 + 옷 종류 명사를 반드시 포함한 짧은 한국어 이름(예: '검정 슬랙스', '회색 후드티'). ",
        "종류 명사를 생략하지 말 것(이 이름으로 카테고리를 교차검증한다). ",
        "category: 반드시 [상의, 하의, 아우터, 니트, 원피스, 신발, 가방] 중 하나이고 name 의 종류 명사와 일치해야 한다. ",
        "color: 한국어 색 이름, material: 추정 소재(모르면 빈 문자열), ",
        "fit: 핏·실루엣을 한 단어로(슬림·레귤러·루즈·오버핏·와이드·크롭·롱·스트레이트·테이퍼드 등). ",
        "색과 종류가 비슷해도 핏이 다르면 서로 다른 옷이므로 fit 을 구분해 적는다. 판단이 어려우면 빈 문자열. ",
        "folded: 그 옷이 접혀 있거나 개켜져 있으면 true, 걸려 있거나 펼쳐져 있으면 false. ",
        "box: 그 아이템 하나가 사진에서 차지하는 최소 사각 영역. 사진 왼쪽 위가 원점이고 x,y,w,h 를 사진 크기 대비 백분율 정수(0~100)로 적는다. ",
        "box 는 해당 아이템만 최대한 타이트하게 감싸야 하고 다른 아이템을 포함하면 안 된다. 서로 다른 항목이 같은 box 를 쓰면 안 된다.\n",
        "반드시 아래 JSON 으로만 답한다: {\"items\": [{\"name\": string, \"category\": string, \"color\": string, \"material\": string, \"fit\": string, \"folded\": boolean, \"box\": {\"x\": int, \"y\": int, \"w\": int, \"h\": int}}]}"
    );
    let user_text = concat!(
        "이 사진에 있는 옷을 한 벌씩 빠짐없이 분리해 목록으로 만들어줘. ",
        "겹쳐 걸린 옷·개켜 쌓인 옷도 각각 항목으로 세고, 접혀 있는 바지를 상의로 착각하지 마. ",
        "각 아이템의 box 영역을 정확히 표시하고 액세서리는 빼줘."
    );
    let parsed = chat_json_messages(vision_messages(system, user_text, image_data_url), 3200, 0.2, 26).await?;
